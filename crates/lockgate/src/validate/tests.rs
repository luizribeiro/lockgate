...
fn rejects_nested_streams_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:compound-wrappers/stream-api#run`: offending type `stream` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
...
fn rejects_streams_nested_in_records() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/record-api#run`: offending type `stream` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
...
fn rejects_futures_nested_in_lists() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/list-api#run`: offending type `future` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
...
fn rejects_futures_nested_in_fixed_length_lists() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/fixed-list-api#run`: offending type `future` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
...
fn rejects_streams_nested_in_results() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/result-api#run`: offending type `stream` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
...
fn rejects_futures_nested_in_maps() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/map-api#run`: offending type `future` cannot cross an invocation boundary; return value data instead, keep durable state behind a host capability, or use a future scoped invocation feature. Async signatures are often generated by guest toolchains by default; the exported WIT function must be a synchronous value-returning function, but the host call may still be asynchronous."
    );
}
