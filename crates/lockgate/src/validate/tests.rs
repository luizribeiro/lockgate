...
fn rejects_world_level_exported_functions_with_guidance() {
    ...
    assert_eq!(
        ...
        "unsupported export `root#run`: offending type `world-level exported function` cannot cross an invocation boundary"
    );
}
...
fn rejects_world_level_exported_types_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `root#<type payload>`: offending type `world-level non-interface export` cannot cross an invocation boundary"
    );
}
...
fn rejects_unused_exported_resource_declarations_with_guidance() {
    ...
    assert_eq!(
        message,
        "unsupported export `test:unused-resource/api#<type file>`: offending type `resource file` cannot cross an invocation boundary"
    );
...
fn rejects_unused_use_aliased_resource_declarations() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:alias-unused/api@0.1.0#<type file>`: offending type `resource file` cannot cross an invocation boundary"
    );
}
...
fn rejects_unused_multi_hop_resource_aliases() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:alias-multi/api@0.1.0#<type document>`: offending type `resource file` cannot cross an invocation boundary"
    );
}
...
fn rejects_nested_owned_handles_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:owned-handle/api#run`: offending type `own<file>` cannot cross an invocation boundary"
    );
}
...
fn rejects_nested_borrowed_handles_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:borrowed-handle/api#run`: offending type `borrow<file>` cannot cross an invocation boundary"
    );
}
...
fn rejects_nested_futures_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:compound-wrappers/future-api#run`: offending type `future` cannot cross an invocation boundary"
    );
}
...
fn rejects_nested_streams_with_guidance() {
    ...
    assert_eq!(
        validate_value_only_exports(&bytes).unwrap_err().to_string(),
        "unsupported export `test:compound-wrappers/stream-api#run`: offending type `stream` cannot cross an invocation boundary"
    );
}
...
fn reports_nested_error_context_before_component_compilation() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/error-context-api#run`: offending type `error-context` cannot cross an invocation boundary"
    );
}
...
fn rejects_streams_nested_in_records() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/record-api#run`: offending type `stream` cannot cross an invocation boundary"
    );
}
...
fn rejects_owned_handles_nested_in_variants() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/variant-api#run`: offending type `own<file>` cannot cross an invocation boundary"
    );
}
...
fn rejects_futures_nested_in_lists() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/list-api#run`: offending type `future` cannot cross an invocation boundary"
    );
}
...
fn rejects_futures_nested_in_fixed_length_lists() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/fixed-list-api#run`: offending type `future` cannot cross an invocation boundary"
    );
}
...
fn rejects_error_contexts_nested_in_tuples() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/tuple-api#run`: offending type `error-context` cannot cross an invocation boundary"
    );
}
...
fn rejects_streams_nested_in_results() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/result-api#run`: offending type `stream` cannot cross an invocation boundary"
    );
}
...
fn rejects_futures_nested_in_maps() {
    ...
    assert_eq!(
        error.to_string(),
        "unsupported export `test:compound-wrappers/map-api#run`: offending type `future` cannot cross an invocation boundary"
    );
}

#[test]
fn exposes_stable_accessors_for_unsupported_exports() {
