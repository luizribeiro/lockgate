...
fn missing_component_reports_the_plugin_build_command() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_workbench-host"))
        .arg(missing)
        .output()
        .expect("failed to run workbench host");

    assert!(!output.status.success());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("could not read plugin component"));
    assert!(stderr.contains(
        "nix develop -c cargo build --manifest-path examples/workbench/plugins/tidy/Cargo.toml \
